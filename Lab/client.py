...
class ImportClient:
    ...
    def get_categories(self):
        ...
        if response.status_code == 200:
            ...
            print('Failed to retrieve category hierarchy paths:',
    ...
    def get_category_ids(self):
        ...
        cat_list = []
        for i in range(5):
            row = categories_json['rows'][i] if i < len(
                categories_json['rows']) else None
            if row:
    ...
    def publish_metadata_enrichment_assets(self, metadata_enrichment_area_id, publish_scope="all_assets", asset_ids=None, filter_criteria=None):
        ...
        try:
            ...
            print(
                f'Publish Assets Response: {response.status_code}, {response_json}')
            return response_json
        except requests.JSONDecodeError:
            print(
                f'Error in Publish Assets Response: {response.status_code}, {response.text}')
            return None
    ...
    def create_and_run_MDE(self, name, mdi_id, enrichment_assets=None, description=None, enrichImmediate=True, job_name=None, job_schedule=None, publish_job_id=None, publish_job_name=None, publish_job_schedule=None, tags=None):
        ...
        try:
            ...
            try:
                response_json = response.json()
                print(
                    f'{name}: Metadata Enrichment Creation Response: {response.status_code}')
                return response_json
            except requests.JSONDecodeError:
                print(
                    f'Error in Metadata Enrichment Creation Response: {response.status_code}, {response.text}')
                return None
    ...
    def update_mde_settings(self):
        ...
        response = requests.put(endpoint, headers=headers,
                                params=params, json=mde_settings)
    ...
    def get_metadata_import_details(self, metadata_import_id):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to retrieve metadata import details: {response.status_code}")
            print(response.text)
            return None
    ...
    def create_and_run_metadata_import(self, connection_id, paths_scope, name="Metadata Import"):
        ...
        mdi_response = requests.post(
            mdi_url, json=mdi_payload, headers=headers)
        try:
            mdi_response_json = mdi_response.json()
            print(f'MDI Creation Response: {mdi_response.status_code}')
        except requests.JSONDecodeError:
            print(
                f'Error in MDI Creation Response: {mdi_response.status_code}, {mdi_response.text}')
            mdi_response_json = {}
        ...
        if mdi_id:
            ...
            job_response = requests.post(
                job_url, json=job_payload, headers=headers)
            try:
                ...
                print(
                    f'Error in Job Creation Response: {job_response.status_code}, {job_response.text}')
                job_response_json = {}
            ...
            if job_id:
                ...
                try:
                    ...
                    print(
                        f'Error in Patch MDI Response: {patch_response.status_code}, {patch_response.text}')
                    patch_response_json = {}
                ...
                if patch_response.status_code == 200:
                    ...
                    try:
                        ...
                        print(
                            f'Error in Run Job Response: {run_response.status_code}, {run_response.text}')
    ...
    def get_connections(self, bearer):
        ...
        headers = {
            ...
            # Insert the actual bearer token
        ...
        if response.status_code == 200:
            ...
            print(
                f"Authenticated request failed with status code {response.status_code}")
            print(response.text)
    ...
    def define_cos_connection(self, bearer, catalog=True):
        ...
        if response.status_code == 200 or response.status_code == 201:
            print(
                f"Connection to {bold_blue_start}Cloud Object Storage{reset} defined successfully!")
            data = response.json()
            self.cos_id = data.get('metadata', {}).get('asset_id')
        else:
            print(
                f"Failed to define connection with status code {response.status_code}")
            print(response.text)
    ...
    def define_db2_connection(self, bearer, catalog=True):
        ...
        if response.status_code == 200 or response.status_code == 201:
            print(
                f"Connection to {bold_blue_start}DB2 Warehouse{reset} defined successfully!")
            data = response.json()
    ...
    def define_psql_connection(self, bearer, catalog=True):
        ...
        if response.status_code == 200 or response.status_code == 201:
            print(
                f"Connection to {bold_blue_start} PostgreSQL{reset} defined successfully!")
            data = response.json()
    ...
    def import_governance_artifacts(self, zip_file_path):
        ...
        with open(zip_file_path, 'rb') as file:
            files = {'file': file}
            response = requests.post(url, headers=headers, files=files, params={
                                     'merge_option': 'specified'}, verify=False)
            ...
            if response.status_code in [200, 201]:
                ...
                print(
                    f"Failed to import governance artifacts with status code {response.status_code}")
                print(response.text)
                return None
    ...
    def check_import_status(self, process_id):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to check import status with status code {response.status_code}")
            print(response.text)
            return None
    ...
    def main_import_process(self, zip_file_path, process_id=None):
        ...
        if process_id:
            while True:
                ...
                print(
                    f"Governance Artifacts Import Progress: {percent_complete:.2f}%")
    ...
    def get_catalogs(self, bearer):
        ...
        if response.status_code == 200:
            ...
            catalog_info = [{"guid": catalog["metadata"]["guid"],
                             "name": catalog["entity"]["name"]} for catalog in catalogs]
            return catalog_info
        else:
            print(
                f"Failed to get catalogs with status code {response.status_code}")
            print(response.text)
            return None
    ...
    def run_main_import_process(self):
        ...
        db2_mdid, db2_mdi_response = self.create_and_run_metadata_import(
            self.db2_id, path_db2, name="DB2 Metadata Import")
        ...
        cos_mdid, cos_mdi_response = self.create_and_run_metadata_import(
            self.cos_id, path_cos, name="Cloud Object Storage Metadata Import")
        ...
        psql_mdid, psql_mdi_response = self.create_and_run_metadata_import(
            self.psql_id, path_psql, name="PostgreSQL Metadata Import")
        ...
        db2_publish_result = self.publish_metadata_enrichment_assets(
            db2_mde_id)
        print("DB2 Warehouse metadata enrichment assets published!")

        cos_mde_id = cos_result.get("metadata", {}).get("asset_id")
        cos_publish_result = self.publish_metadata_enrichment_assets(
            cos_mde_id)
        print("Cloud Object Storage metadata enrichment assets published!")

        psql_mde_id = psql_result.get("metadata", {}).get("asset_id")
        psql_publish_result = self.publish_metadata_enrichment_assets(
            psql_mde_id)
        print("PostgreSQL metadata enrichment assets published!")
        print("")
        cowsay.stegosaurus(
            "Congratulations! You have successfully imported governance artifacts, add data connections and enriched metadata! You can now begin the Data Product Hub portion of the lab.")
# Example usage:
# client = ImportClient()
# client.verify_vars()
